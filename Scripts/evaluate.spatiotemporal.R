...
dplyr::bind_rows(datalist)  %>%
  ...
  mutate(#geographic_position = Latitude * Longitude,
    ...
    PA = ifelse(catch_pp == 0, 0, 1)) %>%
  dplyr::select(!c(X, Y)) -> sdmData # bind presence dfs, omitting locations extracted
...
model_iter <- function(train, test, seas, sptemp, iteration){
  ...
  saveRDS(model_b, paste0("./Models/New models/lm.modelb.", seas,".", iteration, sptemp, ".rda"))
  ...
  saveRDS(model_p, paste0("./Models/New models/lm.modelp.", seas,".", iteration, sptemp, ".rda"))
  ...
  write.csv(eval_df, paste0("./Models/New models/model_iteration_", sptemp, ".csv"))
...
# 
# write.csv(model_out, "./Models/New models/model_iteration_space.csv")
# read.csv("./Models/model_iteration_space.csv")


1:10 %>%
  map_df(~model_iter(train, test, "F", "spacetime", .x)) -> model_out

write.csv(model_out, "./Models/New models/model_iteration_spacetime.csv")
read.csv("./Models/New models/model_iteration_spacetime.csv")

### COMPARE MODEL RESIDUALS OF BEST NON-SPATIOTEMPORAL MODEL and SPATIOTEMPORAL MODELS--------------------------
  # Load best models for each scenario
  best.b <- readRDS("./Models/New models/lm.modelb.F.6.rda") # non-spatiotemporal
  best.p <- readRDS("./Models/New models/lm.modelp.F.6.rda") # non-spatiotemporal
  # 
  # b.space <- readRDS("./Models/lm.modelb.F.6space.rda") # spatial
  # p.space <- readRDS("./Models/lm.modelp.F.6space.rda") # spatial
  
  b.spacetime <- readRDS("./Models/New models/lm.modelb.F.5spacetime.rda") # spatiotemporal
  p.spacetime <- readRDS("./Models/New models/lm.modelp.F.5spacetime.rda") # spatiotemporal
  ...
  cbind(train %>% filter(iter == 5), resid = resid.b.spacetime) -> rr
...
purrr::map_df(~eval_spatresid(b.spacetime, p.spacetime, "spacetime", train, test, preds, 5, observ, .x)) -> out.st

c(1998:2019, 2021:2023) %>%
  purrr::map_df(~eval_spatresid(best.b, best.p, "env", train, test, preds, 6, observ, .x)) -> out.env

write.csv(out.st, "./Output/New models/resid.df.spacetime.csv")
write.csv(out.env, "./Output/New models/resid.df.env.csv")

# Load data
out.st <- read.csv("./Output/New models/resid.df.spacetime.csv")
out.env <- read.csv("./Output/New models/resid.df.env.csv")

# Load models 
best.b <- readRDS("./Models/New models/lm.modelb.F.6.rda") # non-spatiotemporal
best.p <- readRDS("./Models/New models/lm.modelp.F.6.rda") # non-spatiotemporal

# b.space <- readRDS("./Models/New models/lm.modelb.F.6space.rda") # spatial
# p.space <- readRDS("./Models/New models/lm.modelp.F.6space.rda") # spatial
...
eval_models(best.b, best.p, train, test, 6) -> eval.env

eval_models(b.spacetime, p.spacetime, train, test, 5) -> eval.st
...
ggplot(rr) + 
  ...
  coord_sf(xlim = plot.boundary$x,
           ylim = plot.boundary$y)+
  ggtitle("Env (AUC=0.94, Spearman's rho=0.48, PDE=0.37)")+
  theme_bw() + 
  theme(axis.title = element_text(size = 10),
        title = element_text(size = 10),
...
ggplot(rr) + 
  ...
  coord_sf(xlim = plot.boundary$x,
           ylim = plot.boundary$y)+
  ggtitle("Env+Space+Time (AUC=0.93, Spearman's rho=0.47, PDE=0.37)")+
  theme_bw() + 
  theme(axis.title = element_text(size = 10),
        title = element_text(size = 10),
        axis.text = element_text(size = 6),
